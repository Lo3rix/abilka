...
const ResultsTypingTest = ({ selectedTest, results, setResults }) => {
        ...
        <ul>
          {displayedResults.map(result => (
            <li key={result.id}>
              ...
              CPM: {result.score}, Время: {result.average_time} сек, Дата: {new Date(result.created_at).toLocaleString()}
            </li>
          ))}
        </ul>
      ) : (
...
export default ResultsTypingTest;
